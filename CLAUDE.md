...
- **Stage transitions via Anthropic tool-use.** Claude has access to a `set_stage(next_stage, reason)` tool defined in `lib/ai/tools.ts`. When Claude judges the current stage complete, it calls the tool — server-side `execute` updates the DB and triggers async profile extraction. The tool call doesn't appear in user-visible text. **Empirically verified working** (`npx tsx scripts/e2e-test-chat.ts stage` drives onboarding → interests; the model calls `set_stage` on turn 2 once all three onboarding criteria are supplied). The two factors that resolved the earlier "model won't call the tool" pain are both in place: (a) `stopWhen: stepCountIs(2)` on `streamText` so the second step where the tool actually executes can run, and (b) the strong "REQUIRED stage advancement mechanism … you MUST call this tool" language in `makeSetStageTool`'s description plus the per-stage prompt's "**חובה לקרוא לכלי set_stage**" directive.
- **Per-stage system prompts**: `lib/ai/prompts/stages/*.ts`. `assembleSystemPrompt(stage)` in `lib/ai/prompts/system.ts` composes the base prompt + stage overlay. The composed prompt is wrapped by `getCachedSystemMessage(stage)` (in `lib/ai/client.ts`) as a `SystemModelMessage` carrying `providerOptions.anthropic.cacheControl: ephemeral`, and passed to `streamText` via the `system:` parameter (NOT inside `messages` — AI SDK v6 warns against system-in-messages, and the SDK preserves `providerOptions` on system messages passed via the dedicated parameter). With ~3000 input tokens we cross Sonnet 4.6's cache threshold; **empirically verified that cache_read_tokens ≥ 0 on turn 1 and ≥ ~2500 on turn 2+ of any conversation** (test via `npx tsx scripts/e2e-test-chat.ts`).
...
- **Forced-wrap escalation** when the model refuses to call `wrap_up` past `max_questions`: server runs a feedback-only `generateText({ output: Output.object(...) })` repair call against the transcript (`runWrapRepairCall` in `lib/interview/tools.ts`). If THAT fails, fall back to a templated `feedback_summary_he`. `interview_sessions.forced_wrap` flags both paths for auditing. AI SDK v6 deprecated `generateObject` — use `generateText` with `output: Output.object({schema})` going forward.
...
- **Target role resolution is server-side**: client sends `target_occupation_id` (catalog) OR `target_role_he` (free text), never both; server resolves the canonical Hebrew display name via `loadAllOccupations()` + lookup by id, using `Occupation.title_he` (not `name_he`). Skill names for the technical persona overlay are resolved through `content/skills/taxonomy.json` keyed by `skill_id`.
- **Resume model**: `/interview` always opens at the picker + history list (latest 5 sessions). An in-progress session is reachable by clicking its row; a completed session shows the WrapUpScreen. There's no auto-resume — each visit is a fresh decision. Added `getLatestRecommendationForUser(userId)` to `lib/db/recommendations.ts` (no TTL check, no profile_hash filter) so the picker can show stale-but-relevant top-5 roles without re-computing.
...
- **Action-suffix on client callback props**: Next.js 16 RSC requires it (verified via `next-lint` warning code 71007). PersonaSelector's callback is `onChangeAction`, TargetRolePicker's are `onSelectOccupationAction` and `onChangeFreeTextAction`. Same convention as `CvReview`'s `onSaveAction`/`onCancelAction`.

## Phase 6c architecture (production-readiness fixes + observability + polish)

Bundles 8 production-readiness bug fixes (Bucket A, surfaced by the 2026-05-17 deep review) with observability + a11y + state audit + mobile QA (Bucket B). A landed first so Sentry doesn't ship to production silencing the bugs we already know about.

- **Chat-engine message-flow fix** (A1, regression from 6a commit `9033071`): the chat route now appends `{role:"user", content:userText}` to `historyAsModelMessages` before calling `streamLlmTurn`. The engine intentionally stays minimal — it doesn't auto-append because the interview route already builds its own preamble-wrapped message. The unit test in `tests/unit/ai/engine-message-flow.test.ts` pins the contract so future engine refactors don't regress this.
- **SECURITY DEFINER privileges revoked** (A2): `merge_career_profile` + `increment_conversation_counters` previously had EXECUTE granted to PUBLIC. Anon/authenticated clients with the publishable key could call them directly. Migration `20260517100000_revoke_rpc_execute.sql` revokes from `public, anon, authenticated` and grants only to `service_role`. Audit before adding any new SECURITY DEFINER function: it needs an explicit REVOKE in the migration to avoid the same hole.
- **Server-side consent enforcement** (A3): `lib/consent.ts` exports `requireConsent(userId, purposes?)` that throws `NoConsentError` if any required purpose lacks an active row in `public.consents`. Called at the top of every gated API route (chat/cv/cv-confirm/recommendations/plan/interview/interview-wrap). The client modal is now UX; the server enforces correctness. Default required purposes: `processing` + `disclaimer`. Marketing + training_data are opt-in only.
- **CV upload race fix** (A4): the real race was a stale-closure read of `reviewData?.cvUploadId` inside `onFinish`. Fixed via `useRef<string>("")` written immediately after upload-response validation, plus a `saveDisabled` prop wired into `CvReview`'s submit button predicate, plus a defensive guard inside `handleConfirm`. Belt + suspenders.
- **Extraction shape alignment** (A5): chat-extracted `values` are now a `string[]` of canonical keys (not `{key, label_he}` objects); `constraints` field names match `MatchingProfile` exactly, including `risk_tolerance` (not `risk_tolerance_1_10`) and `english_level` enum without `native`. `buildMatchingProfile` consumes the matcher shape directly — no transition shim needed.
- **CV confirm scoped UPDATE** (A6): refactored to `mergeCvSkillsIntoLatestProfile(userId, skills)` which orders by `updated_at desc, limit 1` for the read and scopes UPDATE to that row's `id`. Avoids overwriting every career_profile row when a user has multiple conversations.
- **Recommendations regen bypasses server cache** (A7): server route accepts `{force: true}` in body that skips `getCached`. Client sends it on regenerate (the prior `cache: "no-store"` only affected HTTP cache, not the route's DB-level cache).
- **Sentry server + edge only** (B1): `@sentry/nextjs` configured with `sendDefaultPii: false` and aggressive `beforeSend` that drops `event.request.data`, `event.request.cookies`, `event.request.headers` and PII from `event.user`. Source maps via `withSentryConfig` at build time (`sourcemaps.deleteSourcemapsAfterUpload: true`). NO client SDK, NO performance tracing, NO replay — minimal pre-launch observability. Engine's `onError` also calls `Sentry.captureException` with `tags: { context }` + `extra: { contextId }`. Browser SDK + Performance can land in 6c.5 once production traffic exists.
- **All-surfaces smoke test** (B2): `scripts/verify-all-surfaces.mjs` runs 14 surfaces × 3 viewports (375/768/1280) with `@axe-core/playwright` per combination. Exits non-zero on critical/serious a11y violations, pageerror, or console errors. The script POSTs `/api/consent` after first navigation so `/recommendations` renders properly instead of 403-ing.
- **WCAG 2.2 AA verified**: zero critical/serious axe violations across all surfaces. Focus-visible rings added to 9 custom `<button>` elements that weren't using the shadcn `<Button>` component. Keyboard nav verified manually.
- **State audit completed** across chat (typing indicator added), cv (i18n error keys realigned to API's snake_case codes), assessment (persistent inline error banners on submission failure), plan (animated loading + generation indicators + toast on toggle failure). Verified `/recommendations` and `/interview` already had solid states from Phases 4/6a.
