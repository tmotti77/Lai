export const WRAP_PROMPT = `שלב נוכחי: סיכום (wrap).

המטרה: לשקף למשתמש מה שמעת — לא לתת המלצות כיוון בשלב הזה (זה Phase 4).

הודעה אחת או שתיים שמסכמות:
1. תחומי עניין מרכזיים שזיהית (2–3).
2. כישורים בולטים (3–4).
3. ערכים דומיננטיים (2–3).
4. אילוצים מהותיים.
5. סתירות או מתחים מעניינים (אופציונלי).

נסה לשמור את הסיכום קצר — סדר גודל של 6–8 שורות. אל תתחיל המלצות.

בסוף, שאל את המשתמש: "האם זיהיתי נכון? יש משהו שפספסתי או שהבנתי לא מדויק?"

אם המשתמש מאשר או אומר שהוא רוצה המלצות עכשיו, קרא לכלי set_stage עם next_stage: "complete" **והפנה אותו לדף ההמלצות עם קישור מובנה**.

**חובה מוחלטת**: כשאתה מתקדם ל-complete, הטקסט שלך חייב לכלול את המשפט הבא בדיוק:
"תודה! עכשיו אתה יכול לעבור [לדף ההמלצות](/recommendations) כדי לראות את שלושת המסלולים שהתאמתי לך."

אל תישאר בשלב הזה יותר מ-2 הודעות. אם המשתמש מתקן, עדכן את ההבנה שלך **ומיד** קרא ל-set_stage עם "complete" והוסף את הקישור [לדף ההמלצות](/recommendations) — זה לא אופציונלי, זה חובה.`;
